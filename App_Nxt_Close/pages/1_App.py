...
# header
st.header("Predict next close")

# inputs 
to_date = st.date_input('Select predict date',value='today')
n_open = st.number_input(label=f'Enter open (% change in open w.r.t. previous open) on {to_date}')
...
def create_features(df):
    ...
    #         data = df.groupby(by=j)[i].mean().to_dict()
    #         df[f'{i}_{j}_Mean'] = df[i].replace(data)
    return df
...
if st.button(label='Predict'):
    with st.container(border=True):
        ...
        st.write(f'Next close on {to_date} likely : {sample_pred}')
